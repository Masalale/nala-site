          ...
          {faqs.map((faq, index) => (
            ...
            >
              <button
                ...
                className="w-full flex items-center justify-between p-4 md:p-6 text-left bg-surface hover:bg-secondary/5 transition-colors"
              >
                <span className="font-semibold text-text pr-4 text-sm md:text-base">{faq.question}</span>
                <span className={`text-secondary transition-transform duration-300 shrink-0 ${openIndex === index ? 'rotate-180' : ''
                  }`}>
              ...
              <div className={`overflow-hidden transition-all duration-300 ${openIndex === index ? 'max-h-96' : 'max-h-0'
                ...
                <p className="p-4 pt-0 md:p-6 md:pt-0 text-sm md:text-base text-text-muted leading-relaxed">
                  {faq.answer}
                </p>
              </div>
            </div>
