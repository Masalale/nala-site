...
export function Process() {
  ...
  return (
    <section id="process" className="py-16 md:py-24 bg-surface">
        ...
        <div className="relative">
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-8">
            {steps.map((step, index) => (
              <div
                key={step.number}
                className={`relative group bg-background/40 backdrop-blur-sm border border-secondary/10 p-4 rounded-2xl md:bg-transparent md:backdrop-blur-none md:border-none md:p-0 md:rounded-none text-left md:text-center transition-all duration-500 hover:border-secondary/30 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                  }`}
